...
reg_s ov7725_eagle_reg[] =
{
    ...
    {OV7725_COM4         , 0x41}, //  C1    41    81       C1
    {OV7725_CLKRC        , 0x00}, //  02    00    00       00
    {OV7725_COM2         , 0x03},
    {OV7725_COM3         , 0xD0},
    {OV7725_COM7         , 0x40},
    {OV7725_HSTART       , 0x3F},
...
uint8 ov7725_eagle_init(uint8 *imgaddr)
{
    ...
    while((ov7725_eagle_reg_init() == 0)&&Timm--);
...
void ov7725_eagle_port_init()
{
    ...
    dma_portx2buff_init(CAMERA_DMA_CH, (void *)&PTC_B1_IN, (void *)ov7725_eagle_img_buff, PTC6, DMA_BYTE1, CAMERA_DMA_NUM, DADDR_KEEPON);
	//PTB_B0_IN   PTB_B2_IN
    ...
    port_init(PTC6, ALT1 | DMA_FALLING | PULLDOWN );         //PCLK

    port_init(PTC7, ALT1 | IRQ_RISING  | PULLDOWN | PF);     //场中断，上拉，上降沿触发中断，带滤波
}
 ...
 *  @since      v5.0
*************************************************************************/
__ramfunc void ov7725_eagle_vsync(void)
{
    //场中断需要判断是场结束还是场开始
  ...
  #else
        PORTC_ISFR = 1 <<  PT6;            //清空PCLK标志位
        dma_repeat(CAMERA_DMA_CH, (void *)&PTC_B1_IN, (void *)ov7725_eagle_img_buff,CAMERA_DMA_NUM);
  #endif
	}
    else                                        //图像采集错误
    {
 ...
 *  @since      v5.0
*************************************************************************/
__ramfunc void ov7725_eagle_dma()
{
 ...
 *  @since      v5.0
*************************************************************************/
__ramfunc void ov7725_eagle_get_img()
{
