...
//  152         DMA_count_Init(DMA_CH4, PTA6 , 0x7FFF, DMA_rising); //E26  dma初始化编码器用
        ...
        BL       DMA_count_Init
//  153     //DMA_count_Init(DMA_CH14, PTB18 , 0x7FFF, DMA_rising);//B18  dma初始化编码器用
//  154     
//  155 //    gpio_init (PTA13, GPI,1);//编码器方向
//  156 //    gpio_init (PTB19, GPI,1);//编码器方向
//  157     
//  158 
//  159     
//  160     //AS5040_Init();
...
//  164     //adc_init(ADC1_SE17);//电池电压初始化
//  165 
//  166     
...
//  171     
//  172     
//  173     pit_init_ms(PIT0,1);    //1ms定时中断   
...
//  174     enable_irq (PIT0_IRQn);
        ...
        BL       NVIC_EnableIRQ
//  175     pit_init_ms(PIT1,20);   //20ms定时中断   
        LDR.N    R0,??DataTable24_5
        LDR      R1,[R0, #+0]
        MOVS     R0,#+20
        MULS     R1,R0,R1
...
//  176     enable_irq (PIT1_IRQn);
        MOVS     R0,#+69
          CFI FunCall NVIC_EnableIRQ
        BL       NVIC_EnableIRQ
//  177     //pit_init_ms(PIT2,500); //500ms定时中断   
//  178      //enable_irq (PIT2_IRQn);
//  179     pit_init_ms(PIT3,1000); //1s定时中断    
        LDR.N    R0,??DataTable24_5
        LDR      R1,[R0, #+0]
        MOV      R0,#+1000
        MULS     R1,R0,R1
...
//  180     enable_irq (PIT3_IRQn);
        ...
        BL       NVIC_EnableIRQ
//  181  
//  182  	NVIC_SetPriority(PIT0_IRQn,0);//1 ms  设置中断优先级
        MOVS     R1,#+0
        MOVS     R0,#+68
          CFI FunCall NVIC_SetPriority
        BL       NVIC_SetPriority
//  183     NVIC_SetPriority(PIT1_IRQn,1);//20ms  设置中断优先级
        MOVS     R1,#+1
        MOVS     R0,#+69
          CFI FunCall NVIC_SetPriority
        BL       NVIC_SetPriority
//  184  	//NVIC_SetPriority(PIT2_IRQn,1);//500ms 设置中断优先级
//  185  	NVIC_SetPriority(PIT3_IRQn,2);//1s    设置中断优先级
        MOVS     R1,#+2
        MOVS     R0,#+71
          CFI FunCall NVIC_SetPriority
        BL       NVIC_SetPriority
...
//  187     
//  188  /*摄像头初始化*/
//  189  //   camera_init(imgbuff);//摄像头初始化
//  190  
//  191  /* PWM */
//  192       
//  193         
...
//  197      ftm_pwm_init(FTM1, FTM_CH1,200, 2250);  
        ...
        BL       ftm_pwm_init
//  198      
//  199      gpio_init (PTC2, GPO,1);//电机使能
        MOVS     R2,#+1
        MOVS     R1,#+1
        MOVS     R0,#+66
          CFI FunCall gpio_init
        BL       gpio_init
//  200 //     FT_PWM_QuickInit(FTM0_CH3_PC04,kPWM_EdgeAligned,10000);    //电机
//  201 //     FTM_PWM_QuickInit(FTM0_CH2_PC03,kPWM_EdgeAligned,10000);    //电机
//  202 //     FT_PWM_QuickInit(FTM1_CH1_PA09,kPWM_EdgeAligned,200);      //舵机
...
//  223     
//  224     //MMA8451_Init();
//  225     OLED_Init();              //OLED初始化
          CFI FunCall OLED_Init
        BL       OLED_Init
//  226     GUI_SetColor(1,0);        //此时是正常显示，若(0,1)则反色显示	
        MOVS     R1,#+0
        MOVS     R0,#+1
...
//  228     GUI_PutString(0,0,Str_View);
        ...
        BL       GUI_PutString
//  229     GUI_Exec();               //更新一次屏幕缓冲
          CFI FunCall GUI_Exec
        BL       GUI_Exec
//  230     
//  231     //uart_init(UART0,115200);   //串口0初始化
//  232 
//  233     EnableInterrupts;	//打开中断
        CPSIE    I
//  234 }
        POP      {R0,PC}          ;; return
...
Data_Get:
        PUSH     {R7,LR}
          CFI R14 Frame(CFA, -4)
          CFI CFA R13+8
//  254            Flash_Read_Plan(Flash_Data_Plan,Plan_Num,-1);                  //读取Flash部分变量
        MOVS     R2,#-1
        ...
        BL       Flash_Read_Plan
//  255            
//  256 		   PID_ANGLE.P = Flash_Data_Plan[0]*0.1;                          //直立P
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+0]
        ...
        STR      R0,[R1, #+0]
//  257 		   PID_ANGLE.D = Flash_Data_Plan[1]*0.01;                         //直立D
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+4]
          CFI FunCall __aeabi_ui2d
        BL       __aeabi_ui2d
        ...
        STR      R0,[R1, #+16]
//  258                                                                           
//  259            PID_TURN.P  = Flash_Data_Plan[2];                              //转弯P
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+8]
        ...
        STR      R0,[R1, #+0]
//  260            PID_TURN.I  = Flash_Data_Plan[3];                              //中线补偿
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+12]
          CFI FunCall __aeabi_ui2f
        BL       __aeabi_ui2f
        LDR.N    R1,??DataTable1_8
        STR      R0,[R1, #+8]
//  261            PID_TURN.D  = Flash_Data_Plan[4];                              //转弯D
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+16]
          CFI FunCall __aeabi_ui2f
        BL       __aeabi_ui2f
        ...
        STR      R0,[R1, #+16]
//  262                                                                           
//  263            PID_SPEED.P = Flash_Data_Plan[5]*0.1;                          //速度P
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+20]
          CFI FunCall __aeabi_ui2d
        BL       __aeabi_ui2d
        ...
        STR      R0,[R1, #+0]
//  264            PID_SPEED.I = Flash_Data_Plan[6]*0.1;                              //速度I
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+24]
          CFI FunCall __aeabi_ui2d
        BL       __aeabi_ui2d
        ...
        LDR.N    R1,??DataTable1_9
        STR      R0,[R1, #+8]
//  265            SetSpeed    = Flash_Data_Plan[7];                              //设定的速度(高速)
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+28]
        LDR.N    R1,??DataTable1_10
        STR      R0,[R1, #+0]
//  266                                                                        
//  267            SpeedMini   = Flash_Data_Plan[8];                              //设定的中速
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+32]
        LDR.N    R1,??DataTable1_11
        STR      R0,[R1, #+0]
//  268            SpeedMid    = Flash_Data_Plan[9];                              //设定的低速
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+36]
        LDR.N    R1,??DataTable1_12
        STR      R0,[R1, #+0]
//  269            
//  270            QianZhan    = Flash_Data_Plan[10]>50?50:Flash_Data_Plan[10];  //摄像头提取前瞻
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+40]
        CMP      R0,#+51
        BCC.N    ??Data_Get_0
...
??Data_Get_0:
        ...
        LDR.N    R1,??DataTable1_13
        STRB     R0,[R1, #+0]
//  271            ShiZiFind   = Flash_Data_Plan[11];                            //十字和环路识别前瞻
??Data_Get_1:
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+44]
...
//  273            Seagull_Num = Flash_Data_Plan[12]; 
        ...
        STR      R0,[R1, #+0]
//  274            
//  275            HowMuch_Ring_Finds= Flash_Data_Plan[13];                      //识别多少次环路算真环路
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+52]
        LDR.N    R1,??DataTable1_16
        STR      R0,[R1, #+0]
//  276            How_Much_To_Find_Ring = Flash_Data_Plan[14];                  //海鸥阈值
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+56]
        LDR.N    R1,??DataTable1_17
        STR      R0,[R1, #+0]
//  277            
//  278            Forward_Safe_Angle  = Flash_Data_Plan[15];                    //前保护角度限值
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+60]
        LDR.N    R1,??DataTable1_18
        STR      R0,[R1, #+0]
//  279            Backward_Safe_Angle = Flash_Data_Plan[16] ;                   //后保护角度限值
        LDR.N    R0,??DataTable1_2
        LDR      R0,[R0, #+64]
...
Data_Public_Get:
        PUSH     {R7,LR}
          CFI R14 Frame(CFA, -4)
          CFI CFA R13+8
//  287    Flash_Read_Plan(Flash_Data_Public,0,-1);         //读取Flash公共部分变量
        MOVS     R2,#-1
        MOVS     R1,#+0
...
//  288 
//  289    //公共部分第0位 初始方案号码
//  290    Plan_Num=Flash_Data_Public[28];                  //获取当前的方案号
        LDR.N    R0,??DataTable24_8
        LDR      R0,[R0, #+112]
...
??Data_Public_Get_1:
        ...
        STR      R0,[R1, #+112]
//  293    
//  294    //公共部分第一位 debug开关选择
//  295    Debug_Button_V=Flash_Data_Public[29]>65530?0:Flash_Data_Public[29];            //更新整体调试信息
        LDR.N    R0,??DataTable24_8
        LDR      R0,[R0, #+116]
        MOVW     R1,#+65531
        CMP      R0,R1
...
??Data_Public_Get_3:
        ...
        STR      R0,[R1, #+0]
//  296    
//  297    Debug_Button_Led      =(Debug_Button_V>> 0)&0x01;       //Led_灯开关
??Data_Public_Get_4:
        ...
        STRB     R0,[R1, #+0]
//  298    //Debug_Button_KEY_BEEP =(Debug_Button_V>> 1)&0x01;       //按键声音开关
//  299    Debug_Bluetooth       =(Debug_Button_V>> 2)&0x01;       //蓝牙调试开关
        LDR.N    R0,??DataTable24_10
        LDR      R0,[R0, #+0]
        UBFX     R0,R0,#+2,#+1
        LDR.N    R1,??DataTable24_12
        STRB     R0,[R1, #+0]
//  300    Debug_Protect         =(Debug_Button_V>> 3)&0x01;       //车体防止其他人乱动开关
        LDR.N    R0,??DataTable24_10
        LDR      R0,[R0, #+0]
        UBFX     R0,R0,#+3,#+1
...
//  301    Debug_CarAngPrt       =(Debug_Button_V>> 4)&0x01;
        ...
        STRB     R0,[R1, #+0]
//  302    Debug_Button_WhatisBeep =(Debug_Button_V>> 5)&0x01;     //啥玩意Beep
        LDR.N    R0,??DataTable24_10
        LDR      R0,[R0, #+0]
        UBFX     R0,R0,#+5,#+1
        ...
        STRB     R0,[R1, #+0]
//  303 
//  304    //公共部分第二位 第三位 陀螺仪加速度计
//  305 
//  306    //anglenum =Flash_Data_Public[0];                  //陀螺仪值
//  307    angular  =Flash_Data_Public[1];                  //加速度计值
        LDR.N    R0,??DataTable24_8
        LDR      R0,[R0, #+4]
...
Data_Public_Save:
        PUSH     {R7,LR}
          CFI R14 Frame(CFA, -4)
          CFI CFA R13+8
//  313    //Flash_Read_Plan(Flash_Data_Public,0,-1);//读取Flash公共部分变量
//  314    //公共部分第0位 初始方案号码
...
//  316    
//  317    
//  318    //公共部分第一位 debug开关选择
//  319    Flash_Data_Public[29]=Debug_Button_V;  //调试开关
        LDR.N    R0,??DataTable24_10
        LDR      R0,[R0, #+0]
        LDR.N    R1,??DataTable24_8
        STR      R0,[R1, #+116]
//  320    
//  321    Debug_Button_Led      =(Debug_Button_V>> 0)&0x01;       //Led_灯开关
        LDR.N    R0,??DataTable24_10
        LDRB     R0,[R0, #+0]
        ANDS     R0,R0,#0x1
        LDR.N    R1,??DataTable24_11
        STRB     R0,[R1, #+0]
//  322    //Debug_Button_KEY_BEEP =(Debug_Button_V>> 1)&0x01;       //按键声音开关
//  323    Debug_Bluetooth       =(Debug_Button_V>> 2)&0x01;       //蓝牙调试开关
        LDR.N    R0,??DataTable24_10
        LDR      R0,[R0, #+0]
        UBFX     R0,R0,#+2,#+1
        LDR.N    R1,??DataTable24_12
        STRB     R0,[R1, #+0]
//  324    Debug_Protect         =(Debug_Button_V>> 3)&0x01;       //车体防止其他人乱动开关
        LDR.N    R0,??DataTable24_10
        LDR      R0,[R0, #+0]
        UBFX     R0,R0,#+3,#+1
        LDR.N    R1,??DataTable24_13
...
//  325    Debug_CarAngPrt       =(Debug_Button_V>> 4)&0x01;
        ...
        STRB     R0,[R1, #+0]
//  326    
//  327    Debug_Button_WhatisBeep =(Debug_Button_V>> 5)&0x01;     //啥玩意Beep
        LDR.N    R0,??DataTable24_10
        LDR      R0,[R0, #+0]
        UBFX     R0,R0,#+5,#+1
        LDR.N    R1,??DataTable24_15
        STRB     R0,[R1, #+0]
//  328    //公共部分第二位 第三位 陀螺仪加速度计
//  329    
...
All_Init:
        PUSH     {R4,LR}
          ...
          CFI CFA R13+8
        MOVS     R4,R0
//  344 	HardwareInit();	//硬件初始化
          CFI FunCall HardwareInit
        BL       HardwareInit
//  345 	ParInit(Way);	//参数初始化
        MOVS     R0,R4
          CFI FunCall ParInit
        BL       ParInit
//  346 	return ;
